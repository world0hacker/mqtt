...
/// <summary>
/// MQTT-SN WILLMSGUPD 报文。
/// 更新遗嘱消息内容。
///
/// 格式:
/// | Length (1-3) | MsgType (1) | WillMsg (n) |
/// </summary>
public sealed class MqttSnWillMsgUpdPacket : IMqttSnPacket
{
    /// <summary>
    /// 获取或设置遗嘱消息内容。
    /// </summary>
    ...
    /// <inheritdoc/>
    public MqttSnPacketType PacketType => MqttSnPacketType.WillMsgUpd;
    ...
    public int WriteTo(Span<byte> buffer)
    {
        ...
        if (WillMessage.Length <= 253)
        {
            ...
            buffer[1] = (byte)MqttSnPacketType.WillMsgUpd;
            offset = 2;
        }
        else
        {
            ...
            buffer[3] = (byte)MqttSnPacketType.WillMsgUpd;
            offset = 4;
        }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static MqttSnWillMsgUpdPacket Parse(ReadOnlySpan<byte> buffer, int length, int headerLength)
    {
        var packet = new MqttSnWillMsgUpdPacket();
