...
/// <summary>
/// MQTT-SN WILLTOPIC 报文。
/// 客户端发送遗嘱主题。
///
/// 格式:
/// | Length (1-3) | MsgType (1) | Flags (1) | WillTopic (n) |
///
/// 空的 WILLTOPIC（长度为 2）表示删除遗嘱。
/// </summary>
public sealed class MqttSnWillTopicPacket : IMqttSnPacket
{
    /// <summary>
    ...
    /// <inheritdoc/>
    public MqttSnPacketType PacketType => MqttSnPacketType.WillTopic;
    ...
    public int Length
    {
        get
        {
            ...
            {
                return 2; // Empty WILLTOPIC
            }

            var topicBytes = Encoding.UTF8.GetByteCount(WillTopic);
            var payloadLength = 1 + topicBytes; // Flags + WillTopic
    ...
    public int WriteTo(Span<byte> buffer)
    {
        ...
        {
            buffer[0] = 2;
            buffer[1] = (byte)MqttSnPacketType.WillTopic;
            return 2;
        }
        ...
        else
        {
            ...
            buffer[3] = (byte)MqttSnPacketType.WillTopic;
            offset = 4;
        }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static MqttSnWillTopicPacket Parse(ReadOnlySpan<byte> buffer, int length, int headerLength)
    {
        var packet = new MqttSnWillTopicPacket();
