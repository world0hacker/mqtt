...
/// <summary>
/// MQTT-SN SUBSCRIBE 报文。
/// 订阅主题。
///
/// 格式:
/// | Length (1-3) | MsgType (1) | Flags (1) | MsgId (2) | TopicIdOrName (2 or n) |
///
/// TopicIdOrName 的长度取决于 Flags 中的 TopicType:
/// - Normal (0x00): TopicName (变长字符串)
/// - Predefined (0x01): TopicId (2 字节)
/// - ShortName (0x02): ShortTopicName (2 字节)
/// </summary>
public sealed class MqttSnSubscribePacket : IMqttSnPacket
{
    /// <summary>
    /// 获取或设置标志位。
    /// </summary>
    ...
    /// <inheritdoc/>
    public MqttSnPacketType PacketType => MqttSnPacketType.Subscribe;
    ...
    public int Length
    {
        get
        {
            ...
            {
                topicLength = 2; // TopicId or ShortName
            }

            var payloadLength = 1 + 2 + topicLength; // Flags + MsgId + Topic
    ...
    public int WriteTo(Span<byte> buffer)
    {
        ...
        if (payloadLength <= 253)
        {
            ...
            buffer[1] = (byte)MqttSnPacketType.Subscribe;
            offset = 2;
        }
        else
        {
            ...
            buffer[3] = (byte)MqttSnPacketType.Subscribe;
            offset = 4;
        }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static MqttSnSubscribePacket Parse(ReadOnlySpan<byte> buffer, int length, int headerLength)
    {
        var dataOffset = headerLength;

        var packet = new MqttSnSubscribePacket
        {
